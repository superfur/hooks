...
import { useDocumentTitle } from '../../src/react';

// React版本的useDocumentTitle示例
// useDocumentTitle example for React
function DocumentTitleExample() {
  // 初始标题值
  // Initial title value
  const [title, setTitle] = useState('React页面 | 初始标题');
  
  // 是否在组件卸载时保留标题
  // Whether to keep the title when component unmounts
  const [keepOnUnmount, setKeepOnUnmount] = useState(false);
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <h1>useDocumentTitle 示例 (React)</h1>
      <p>当前文档标题: <strong>{title}</strong></p>
      
      <div style={{ marginBottom: '15px' }}>
        <label htmlFor="title-input">更改标题: </label>
        <input 
          id="title-input"
          type="text" 
          value={title} 
